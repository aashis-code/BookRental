...
public interface BookTransactionRepo extends JpaRepository<BookTransaction, Integer> {
	...
	@Query(value = "select bt.id,bt.rent_status,bt.created_by ,bt.from_date , bt.to_date ,(bt.to_date::DATE-CURRENT_DATE::DATE) as days_left,\r\n"
			...
			+ "and (?6 is null or bt.rent_status=?6)",
			countQuery = " SELECT \n" +
					"        COUNT(*)\n" +
					"    FROM \n" +
					"        book_transaction bt\n" +
					"    INNER JOIN \n" +
					"        member m ON bt.member_id = m.id\n" +
					"    INNER JOIN \n" +
					"        book b ON bt.book_id = b.id\n" +
					"    WHERE \n" +
					"        bt.created_date BETWEEN COALESCE(?1, bt.created_date) AND COALESCE(?2, bt.created_date)\n" +
					"        AND (?3 IS NULL OR bt.deleted = ?3)\n" +
					"        AND (?4 IS NULL OR bt.book_id = ?4)\n" +
					"        AND (?5 IS NULL OR bt.member_id = ?5)\n" +
					"        AND (?6 IS NULL OR bt.rent_status = ?6)",
			nativeQuery = true)
