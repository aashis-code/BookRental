...
public class MemberImpl implements MemberService {
    ...
    public boolean saveAndUpdateMember(MemberDto memberDto) {
        ...
        if (memberDto.getId() != null) {
            member = memberRepo.findByIdAndDeleted(memberDto.getId(), Boolean.FALSE).orElseThrow(() -> new ResourceNotFoundException("MemberId", String.valueOf(memberDto.getId())));
            ...
            if (roleByName.isEmpty()) {
                role = Role.builder().name("NORMAL").description("This is basic level role assigned for every member.").build();
                roleRepo.save(role);
    ...
    public MemberDto getMemberById(Integer memberId) {
        ...
        Member member = memberRepo.findByIdAndDeleted(memberId, Boolean.FALSE).orElseThrow(() -> new ResourceNotFoundException("Member Id", String.valueOf(memberId)));
    ...
    public boolean assignRoles(Integer memberId, List<String> roles) {
        ...
        Member member = this.memberRepo.findByIdAndDeleted(memberId, Boolean.FALSE).orElseThrow(() -> new ResourceNotFoundException("Member Id", String.valueOf(memberId)));
        ...
        for (String role : roles) {
            ...
            if (!roleNames.contains(upperCaseRole)) {
                Role roleEntity = roleRepo.findByName(upperCaseRole).orElseThrow(() -> new ResourceNotFoundException("Role", upperCaseRole));
